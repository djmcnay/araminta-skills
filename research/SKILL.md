...
|------|-------|----------|-------|
| **Scraping agents** | `mimo-flash` or `gemma4` | Google free API | Cheap, fast, thorough. Never OpenRouter. |
| **Analysis pass** | `mimo-pro` | — | Deep reasoning over collected data |
| **Orchestration & report** | `mimo-omni` (me) | — | Pipeline control, report polish, personality |

### Scraping Agent Availability

Before spawning scraping subagents, check model availability:
1. Try `mimo-flash` first (cheapest)
2. If unavailable, try `gemma4` via Google free API
3. If neither available, surface to David:

> ⚠️ Primary scraping agents unavailable (`mimo-flash` / `gemma4` via Google). How would you like to proceed?
...
Parse David's request to determine:
...
1. If David specifies a vault path → use it
2. If `~/Documents/GitHub/araminta-vault/research/` exists → use it
...
Use `delegate_task` in batch mode (up to 3-4 concurrent). Each subagent gets:
- A focused research mandate
- A list of sources to check
- Output format specification
- Scraping agent model (`mimo-flash` or `gemma4`)
...
- Question the assumptions David has made (does he actually need feature X?)
...
- Find the "I hadn't thought of that" options — things David didn't ask for but might prefer
...
- Output: what David assumed, why it might be wrong, what he should consider instead
- Include source URLs
...
5. **Synthesis:** what actually matters for David's decision?

This pass is done by `mimo-pro` (analytical model), orchestrated by `mimo-omni`.
...
Using the template for the research type, generate the final `report.md`. Orchestration by `mimo-omni` — the report should have personality and voice, not read like a Wikipedia entry.
...
## TL;DR
One paragraph. What is it, should David care, what's the bottom line.
...
Things David might not have considered:
- **Alternative A** — how it compares — Source URL
- **Alternative B** — how it compares — Source URL

## My Assessment
Honest opinion. Not hedged. What would I do. **Challenge David's assumptions here** — if his brief said "must have X" but the research suggests X isn't necessary or Y is better, say so. The report should stretch his thinking, not just confirm what he already believed.
...
|-------|-------|---------------|--------|--------|
| Formal Reviews | mimo-flash | 12 | none | 1 |
| Informal Sentiment | gemma4 | 8 | 2 hallucinations removed | 1 |
| Brand Landscape | mimo-flash | 6 | thin — sent back for more | 2 |
| Alternatives | mimo-flash | 10 | none | 1 |
...
For quick compatibility checks when David is ordering 2-4 components from the same vendor — typically Pi Hut UK accessories (HATs, displays, microphone arrays, speakers, NVMe Base). This is a *lightweight inline review* done directly by reading product pages, not the full multi-agent pipeline.

**Use case signals:** David sends multiple product URLs and asks if they'll work together, or asks "what cables am I missing," or asks to find a better-suited alternative for a specific part in a multi-component build.
...
5. **Power budget** — Pi 5 (up to 3A) + NVMe (~0.5A) + ReSpeaker (~0.1A) + display USB (~0.3A) + HAT (<0.05A) = within 27W/5A PSU, but check peak current for each.
...
8. **Never silently escalate model costs.** If scraping agents are unavailable, ask David.
9. **Challenge the brief.** Don't just find what David asked for — question whether he asked for the right thing. Stretch his thinking like a bendy yoga lady.
