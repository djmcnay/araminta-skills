...
version: 1.0.0
author: Araminta
tags: [contracts, legal-documents, markdown, pdf, html, vault]
...
6. **Generate PDF from HTML with headless Chromium**
   ...
   ```
   Notes:
   - Chromium may emit noisy flag warnings on this machine but still produce the PDF successfully.
   - Verify the output file exists rather than panicking at stderr chatter.

7. **Surface the final file back to the user**
   - Give the vault paths.
...
- Files are saved in the project vault folder, not stranded elsewhere.
