...
When reading configuration files (like your config file or `.env` files), the platform's output filters may automatically redact secrets (e.g., `AGENTMAIL_API_KEY: am_us_...0b25`). This prevents the agent from using those secrets for ad-hoc tool calls or manual configuration.
...
## Pitfalls
- **Sensitive handling:** Keep decoded values local to the task, use them only for the user's requested configuration step, and avoid persisting them in shared notes or committed files.
