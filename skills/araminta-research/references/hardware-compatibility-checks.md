...
the user is ordering 2-4 components from the same vendor (typically retailer) and needs to know if they'll physically work together. This is a **lightweight inline check** — not the full multi-agent research pipeline. Do it yourself by reading product pages and cross-referencing specs.

## Common scenarios

- host machine accessory stack (HAT + display + microphone + speaker + NVMe)
...
- the user sends multiple product URLs and asks "will these work together"
- the user says "check if there's anything else I need" or "what am I missing"
- the user asks for a better option for a specific part in a multi-component order
...
- HDMI displays use the micro-HDMI port. host machine 5 has two micro-HDMI — confirm you're using the right one (HDMI0 is the one nearest the USB-C power).
...
**Power budget:**
- host machine 5 needs a 27W (5V/5A) USB-C PSU. The official host machine 5 PSU delivers this.
- NVMe Base draws power from the host machine's GPIO (no additional PSU).
- ReSpeaker Lite: USB-powered from host machine's USB-A port (~100mA nominal).
- Round HDMI display: needs 5V/300mA via separate USB-C (touch data + power). Can be powered from host machine's USB-A or an external USB-C PSU.
- GPIO HATs (e-ink, etc.): draw power from host machine's GPIO (typically <50mA).
- Total: the host machine's 5V rail must supply the host machine itself (~3A under load) + NVMe (~0.5A) + ReSpeaker (~0.1A) + display USB (~0.3A) + any e-ink HAT (~0.05A) = within budget on a 5V/5A supply. But don't blindly assume — check peak current from each component spec.
...
|-------|------|----|--------------------------|
| micro-HDMI to HDMI | host machine 5 (micro-HDMI) | Round display (HDMI) | retailer Waveshare display pages say not included |
| USB-A to USB-C | host machine 5 (USB-A) | ReSpeaker Lite / display touch | Neither device includes it (they assume USB-C to USB-C or you have one) |
...
**Cable shopping list format:** Deliver as a simple bullet list the user can add to his cart. Each with a brief label: `1x micro-HDMI to HDMI (host machine 5 → display)`, `2x USB-A to USB-C (host machine → ReSpeaker, host machine → display touch)`.
...
|------|------------|-------|
| ReSpeaker Lite | ✅ | USB audio device, host machine 5 detects as sound card |
| Mono Speaker 3W 4Ω | ⚠️ | Connector may not mate — OEM Seeed speaker preferred |
| Round HDMI display | ✅ | HDMI + USB-touch, driver-free |

And a **recommended BOM** that reorders the items the user originally listed, swapping in better-matched alternatives (e.g. OEM speaker instead of generic):
...
6. *(existing parts: host machine 5, NVMe Base, 1TB SSD, Active Cooler)*

### 5. Consider alternatives on the same vendor's site

When the user asks "is there a better option for X," check the vendor's full catalogue for that category before answering. E.g. for speakers: The retailer sells a whole collection of speakers — scan the category page, identify the OEM companion part, and recommend that if it exists.
...
2. **HDMI port numbering on host machine 5.** HDMI0 (nearest the USB-C power port) is the primary display. If the round display is the only HDMI device, either port works. But if adding a second HDMI display later, know which is which.
3. **USB port competition.** host machine 5 has 2 USB-A ports. You need: keyboard/mouse (if using as desktop) + ReSpeaker + display touch = 3 USB devices. Either use a USB hub, or power the display touch from a separate USB-C PSU (the display has its own USB-C input for power + touch data).
